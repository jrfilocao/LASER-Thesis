...
def _get_de_pt_report_entries(sentence_pair_score_threshold, database_cursor):
    entries = []
    entries.append(('language', 'DE-PT'))
    entries.append(('sentence pairs de_pt', str(get_potential_similar_sentences_de_pt(sentence_pair_score_threshold, database_cursor))))
    ...
    entries.append(('unique article pairs de_pt', str(unique_article_pairs_de_pt)))
    entries.append(('unique article pairs with common named entities de_pt', str(unique_article_pairs_with_common_named_entities_de_pt)))
    entries.append(('unique article pairs with common named entities de_pt percentage', str(unique_article_pairs_with_common_named_entities_de_pt_percentage)))
    entries.append(('unique articles with common named entities and multiple similar sentences de_pt',
                               str(unique_articles_with_common_named_entities_and_multiple_similar_sentences_de_pt)))
    entries.append(('unique articles with common named entities and multiple similar sentences de_pt percentage',
                               str(unique_articles_with_common_named_entities_and_multiple_similar_sentences_de_pt_percentage)))
    entries.append(
        ('unique articles with more than 2 sentences de_pt', str(get_unique_articles_with_more_than_2_sentences_de_pt(sentence_pair_score_threshold, database_cursor))))
    return entries


def _get_en_pt_report_entries(sentence_pair_score_threshold, database_cursor):
    entries = []
    entries.append(('language', 'EN-PT'))
    entries.append(('sentence pairs en_pt', str(get_potential_similar_sentences_en_pt(sentence_pair_score_threshold, database_cursor))))
    ...
    entries.append(('unique article pairs en_pt', str(unique_article_pairs_en_pt)))
    entries.append(('unique article pairs with common named entities en_pt', str(unique_article_pairs_with_common_named_entities_en_pt)))
    entries.append(('unique article pairs with common named entities en_pt percentage', str(unique_article_pairs_with_common_named_entities_en_pt_percentage)))
    entries.append(('unique articles with common named entities and multiple similar sentences en_pt',
    ...
    entries.append(('unique articles with common named entities and multiple similar sentences en_pt percentage',
                               str(unique_articles_with_common_named_entities_and_multiple_similar_sentences_en_pt_percentage)))
    entries.append(
        ('unique articles with more than 2 sentences en_pt', str(get_unique_articles_with_more_than_2_sentences_en_pt(sentence_pair_score_threshold, database_cursor))))
    return entries


def _get_en_de_report_entries(sentence_pair_score_threshold, database_cursor):
    entries = []
    entries.append(('language', 'EN-DE'))
    entries.append(('sentence pairs en_de', str(get_potential_similar_sentences_en_de(sentence_pair_score_threshold, database_cursor))))
    ...
    entries.append(('unique article pairs en_de', str(unique_article_pairs_en_de)))
    entries.append(('unique article pairs with common named entities en_de', str(unique_article_pairs_with_common_named_entities_en_de)))
    entries.append(('unique article pairs with common named entities en_de percentage', str(unique_article_pairs_with_common_named_entities_en_de_percentage)))
    entries.append(('unique articles with common named entities and multiple similar sentences en_de',
                               str(unique_articles_with_common_named_entities_and_multiple_similar_sentences_en_de)))
    entries.append(('unique articles with common named entities and multiple similar sentences en_de percentage',
                               str(unique_articles_with_common_named_entities_and_multiple_similar_sentences_en_de_percentage)))
    entries.append(
        ('unique articles with more than 2 sentences en_de', str(get_unique_articles_with_more_than_2_sentences_en_de(sentence_pair_score_threshold, database_cursor))))
    return entries


def _get_total_report_entries(sentence_pair_score_threshold, database_cursor):
    entries = []
    entries.append(('total sentence pairs', str(get_total_sentence_pairs_count(sentence_pair_score_threshold, database_cursor))))
    entries.append(('total unique article pairs', str(get_total_unique_article_pairs_count(sentence_pair_score_threshold, database_cursor))))
    entries.append(('total unique article pairs with common named entities',
    ...
    entries.append(('total unique article pairs with common named entities and more than 2 sentence',
                               str(total_unique_article_pairs_with_common_named_entities_and_more_than_2_sentence)))
    entries.append(('total unique article pairs with more than 2 sentences', str(total_unique_article_pairs_with_more_than_2_sentences)))
    entries.append(('total unique article pairs without common named entities and with more than 2 sentence',
